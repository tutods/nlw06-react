...
const SEO = ({ title, image, description, ...props }: Props) => {
  return (
    <Helmet titleTemplate="Letmeask | %s" defaultTitle="Página Inicial">
      ...
      <meta
        name="description"
        content={description ? description : 'Aplicação desenvolvida no NLW05 da Rocketseat'}
      />
      <meta
        property="og:image"
        content={image || 'https://letmeask-cc61e.web.app/media/letmeask.jpg'}
      />
      <meta property="og:title" content={`${title && `${title} | `}Letmeask`} />
      <meta
